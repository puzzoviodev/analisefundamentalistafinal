...
# 🔹 Parte 3: Indicadores Extras
indicadores_extras = [
    'Valor de mercado', 'Valor da firma', 'Patrimônio Líquido',
    'Ativos', 'Ativo Circulante', 'Disponibilidade',
    'Free Float', 'Tag Along'
]

for indicador in indicadores_extras:
    span_indicador = soup.find('span', string=lambda text: text and indicador in text)
    if span_indicador:
        valor_span = span_indicador.find_next('span')
        valor = valor_span.get_text(strip=True) if valor_span else 'N/A'
